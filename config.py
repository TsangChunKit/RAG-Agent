...
# 相关性阈值：精排分数低于它的片段不进上下文（0 = 关掉这个机制）。
# ⚠️ 量级按本项目实测定标（2026-07-25，bge-reranker-v2-m3 + 本人 57 场逐字稿）：
#   切题问题（"我和妈妈的关系"/"工作压力"/"依戀模式"）top1 ≈ 0.42–0.87，长尾掉到 0.03–0.15；
#   完全无关（"今天天气"0.0035、"巴黎奥运会金牌榜"0.0003）绝大多数在 0.001 量级；
#   但**重叠区真实存在**："怎么修理汽车引擎"有一条 0.10，而切题的"焦虑和睡眠"top1 只有 0.035
#   （语料本身覆盖少）。所以没有干净的分割点，阈值只负责砍掉 0.001 量级的明显噪音尾巴，
#   模糊区交给 min_keep 保底 + prompt 里的低相关警告，而不是硬判。
# **不要照搬通用的 0.5**（那会把绝大多数真实片段都滤掉）。想更严就往 0.03–0.05 调。
RERANKER_MIN_SCORE = 0.01
# 保底条数：只有"一条都没过线"时才退回这么多条最相关的（并在 prompt 里明说"相关性都很低"）。
# 理由是降级要优雅——宁可给 LLM 几条弱材料 + 一句警告，也不要让它在零材料下凭记忆编；
# 反之只要有片段过线就不再凑低分片段（凑数只会稀释注意力）。
RERANKER_MIN_KEEP = 3
