...
**升级 D — 繁简统一(2026-07-25 补)。**
语料是简体(转写工具输出),但使用者习惯打繁体。关键词那条腿靠字符重叠,"水煙"和"水烟"
零重叠 → 繁体提问会检索不到明明存在的记录。规则是**索引和 query 两侧都转简体,展示的原文不动**
(实现见 `scripts/text_norm.py`)。这不是优化,是正确性:不做,繁体提问的召回直接掉到接近零。
...
| 语言 | **Python 3.9**（实际运行 3.9.6） | ✅ 验证 | 代码全程按 3.9 兼容性编写（禁用 `X \| None` 语法）|
...
| 繁简归一化 | `zhconv` | ✅ 已上线 | 纯 Python、无编译依赖;索引与 query 双侧转简体(§1.2 升级 D),缺包时降级为不转换 |
...
| Reranker 重排序 | 中 | 中–高 | **已上线** ✅ |
| 相关性阈值(低分片段不进上下文 + 保底警告) | 低 | 中–高(减少噪音稀释) | **已上线** ✅ |
| 繁简归一化(索引与 query 双侧) | 极低 | 高(繁体提问的召回) | **已上线** ✅ |
| 议题标签体系(职业/关系/消费/自我价值…) | 中 | 中 | v1.5 🔶 |
| 情绪轨迹曲线 | 中 | 中(锦上添花) | v2 ⬜ |
| Agentic RAG(多跳迭代检索) | 高 | 中 | 暂不 ⬛ |
| 知识图谱(心智地图 + GraphRAG 引导检索) | 高 | 中 | **已上线** ✅(超出原计划) |

\* 混合检索标"低",因 BGE-M3 一个模型自带稠密+稀疏,不用额外搭 BM25。

**Reranker 原本放 v1.5,后来提前实装**:业界建议按阶段递进——先建评估框架,再依次加混合检索、
元数据过滤、重排序等越来越贵的步骤。实际跑下来混合检索的排序不够稳,所以提前加了
`bge-reranker-v2-m3` 做精排(`scripts/reranker.py`,「⚙️ 索引设置」里可关)。
精排分数修对之后(见 README §七 的 sigmoid 坑)才有区分度,于是顺势加了**相关性阈值**
(`RERANKER_MIN_SCORE`/`RERANKER_MIN_KEEP`):低于阈值的片段不进上下文,一条都没过线时留几条保底
并在 prompt 里明说"相关性都很低、不要硬套"。阈值设 0 即完全关掉,行为回到改动前。
...
> **现状（2026-07-25）**：系统运行改走 **xAI grok**（默认经本地 Hermes Agent Gateway 代理，也可
> 直连 `api.x.ai`），Gemini provider 暂时停用（Python 3.9 装不到支持 `thinking_level` 的
> google-genai 2.x，详见 README §七）。合规结论不变：**运行时用 API 计费的后端，开发时用 Claude Code
> 订阅**，两者不混用。
...
3. 申请 Gemini API key,放进 `.env`:`GEMINI_API_KEY=...`。
   （现状 2026-07-25:Gemini 停用,改准备 `XAI_API_KEY`;用默认的 Hermes 本地代理时连 key 都不用填,见 §4.1 注）
