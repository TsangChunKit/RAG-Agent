...
这是一个独立的常驻进程，不依赖浏览器标签页是否打开——用法：
    nohup python -m scripts.chat_memory_watcher > /tmp/chat_memory_watcher.log 2>&1 &
...
MARKER_PATH = CHAT_SESSIONS_DIR / ".last_memory_run"


def _latest_session_update() -> datetime | None:
...
def _last_memory_run() -> datetime | None:
...
def _mark_memory_run(ts: datetime) -> None:
    MARKER_PATH.parent.mkdir(parents=True, exist_ok=True)
    MARKER_PATH.write_text(ts.isoformat())


def check_and_update() -> bool:
    """检查一次；如果确实触发了更新就返回 True，方便测试。"""
    latest_update = _latest_session_update()
    if latest_update is None:
        return False

    last_run = _last_memory_run()
    ...
    _mark_memory_run(datetime.now(timezone.utc))
    return True
...
if __name__ == "__main__":
    print(
        ...
        f"空闲 {IDLE_MINUTES} 分钟后自动更新 AI 对话记忆",
        flush=True,
    )
    while True:
        try:
