...
def _load_therapy_graph() -> dict | None:
    if not GRAPH_JSON_PATH.exists():
        return None
    return json.loads(GRAPH_JSON_PATH.read_text(encoding="utf-8"))


def build_chat_graph(sessions: list[dict] | None = None, therapy_graph: dict | None = None) -> dict:
    sessions = sessions if sessions is not None else load_chat_sessions()
    therapy_graph = therapy_graph if therapy_graph is not None else _load_therapy_graph()
...
if __name__ == "__main__":
    ...
    CHAT_GRAPH_JSON_PATH.write_text(json.dumps(graph, ensure_ascii=False, indent=2), encoding="utf-8")
    ...
    print(f"已写入 {CHAT_GRAPH_JSON_PATH}")
