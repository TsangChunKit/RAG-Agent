...
53 份规模小，暴力搜索即可，不建 ANN 向量索引（数据量大了再加 vector 列的 create_index()）。
"""
import json
...
def load_chunks(path=CHUNKS_JSONL_PATH) -> list[dict]:
...
def ingest(chunks: list[dict] | None = None, mode: str = "overwrite"):
    """mode='overwrite' 全量重建；mode='append' 增量追加（供 ingest_new.py 用）。"""
