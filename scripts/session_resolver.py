...
def therapy_dates_ordered() -> list[tuple[str, object]]:
    """[(YYYY-MM-DD, Path), ...] 按时间从早到晚（文件名 14 位前缀天然有序）。实时读盘。"""
    out = []
...
def resolve(question: str) -> dict:
    ...
    if kind == "therapy":
        ordered = therapy_dates_ordered()  # 早→晚
        ...
        ordered = list(reversed(list_sessions()))  # list_sessions 是近→远，反转成早→晚对齐咨询
        picked = _apply_intent(ordered, intent)
        if len(picked) > MAX_FULL_SESSIONS:
            res["overflow"] = True
            picked = picked[-MAX_FULL_SESSIONS:]
...
def _load_topics() -> dict[str, list[str]]:
    """source_file -> topics 列表（来自 summaries/*.json），用于给咨询清单加一句话标签。"""
...
def therapy_manifest() -> str:
    """全部真实咨询记录的极简清单（序号 + 日期 + 主题）。变动低频，可随 static_content 进缓存。"""
    entries = therapy_dates_ordered()
    if not entries:
        return "（暂无真实咨询记录）"
    topics_by_file = _load_topics()
    lines = []
...
def chat_manifest(limit: int = 15) -> str:
    """最近若干次 AI 对话的清单（日期 + 标题）。变动较频，放每轮动态内容、不进缓存。"""
    sessions = list_sessions()  # 近→远
    if not sessions:
        return "（暂无 AI 对话历史）"
    lines = []
...
def render_chat_sessions(session_ids: list[str]) -> str:
    """把指定的 AI 聊天会话渲染成可读逐轮文本，供「读取最近N次对话」时整份塞进上下文。"""
    parts = []
