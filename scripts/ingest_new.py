...
用法：python -m scripts.ingest_new <新文件路径> [--force]
...
def _ensure_in_raw_dir(path: Path) -> Path:
...
def _existing_chunk_source_files() -> set[str]:
...
def ingest_new_file(path: Path, force: bool = False) -> dict:
    path = _ensure_in_raw_dir(path)
    session = parse_transcript(path)

    already_chunked = session.source_file in _existing_chunk_source_files()
    if already_chunked and not force:
        ...
        new_chunks = chunk_session(session)
        chunk_dicts = [asdict(c) for c in new_chunks]

        with open(CHUNKS_JSONL_PATH, "a", encoding="utf-8") as f:
            for c in chunk_dicts:
                f.write(json.dumps(c, ensure_ascii=False) + "\n")

        ingest(chunks=chunk_dicts, mode="append")
    ...
    out_path = summary_path(session.source_file)
...
if __name__ == "__main__":
    ...
    if not args:
        print("用法: python -m scripts.ingest_new <新文件路径> [--force]")
        sys.exit(1)

    for a in args:
        ingest_new_file(Path(a), force=force)
